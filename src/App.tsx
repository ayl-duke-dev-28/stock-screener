...
function Ideas({ universe, priorities, onOpen }: { universe: Stock[]; priorities: MetricKey[]; onOpen: (stock: Stock) => void }) {
  ...
  const themes = [
    ...
    { title: 'Growth at a fair price', tag: 'GARP', copy: 'Above-market earnings growth without the most demanding valuation multiples.', color: 'sand', picks: getRecommendations(universe.filter((s) => exceeds(s.earningsGrowth, 20) && s.pe !== null && s.pe > 0 && below(s.pe, 35) && s.ps !== null && s.ps > 0), ['earningsGrowth', 'pe', 'ps']).slice(0, 3) },
