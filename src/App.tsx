...
import { downloadResultsCsv } from './lib/export'
import {
...
function StockDetail({ stock, priorities, isSaved, onBack, onToggleSave }: { stock: Stock; priorities: MetricKey[]; isSaved: boolean; onBack: () => void; onToggleSave: () => void }) {
  ...
  return <main className="detail-page">
    ...
    <section className="detail-grid">
      ...
      <div className="rating-card card"><span className="eyebrow">Signal rating</span><div className="rating-main"><ScoreBadge score={result.score} size="large"/><div><h2>{scoreToLabel(result.score, result.coverage.ratio)}</h2><p>{result.coverage.available} of {result.coverage.selected} selected factors reported · {result.coverage.ratio >= .8 ? 'High' : result.coverage.ratio >= .5 ? 'Medium' : 'Low'} data confidence</p></div></div><BreakdownBars breakdown={result.breakdown}/></div>
    </section>
...
export default function App() {
  ...
  const filterLabels: Record<NumericFilterKey, string> = {
    minRevenueGrowth: 'Min. revenue growth', minEarningsGrowth: 'Min. earnings growth',
    minFcfGrowth: 'Min. FCF growth', minGrossMargin: 'Min. gross margin',
    maxPe: 'Max. P/E ratio', maxPs: 'Max. P/S ratio',
  }
  const activeCriteria = [
    ...[...activeNumericFilters].map((key) => ({ key, label: `${filterLabels[key]} ${filters[key]}${key === 'maxPe' || key === 'maxPs' ? '×' : '%'}` })),
    ...(filters.sector !== defaultFilters.sector ? [{ key: 'sector' as const, label: filters.sector }] : []),
    ...(filters.marketCap !== defaultFilters.marketCap ? [{ key: 'marketCap' as const, label: `${filters.marketCap} cap` }] : []),
    ...(filters.insiderOnly ? [{ key: 'insiderOnly' as const, label: 'Insider buying' }] : []),
  ]
  ...
  const removeCriterion = (key: NumericFilterKey | 'sector' | 'marketCap' | 'insiderOnly') => {
    if (numericFilterKeys.has(key)) {
      setActiveNumericFilters((current) => {
        const next = new Set(current)
        next.delete(key as NumericFilterKey)
        return next
      })
      return
    }
    if (key === 'sector') patchFilter('sector', defaultFilters.sector)
    if (key === 'marketCap') patchFilter('marketCap', defaultFilters.marketCap)
    if (key === 'insiderOnly') patchFilter('insiderOnly', false)
  }
  ...
  return <div className="app-shell">
    ...
    <main className="dashboard">
      ...
      {view === 'ideas' && <Ideas universe={quotedUniverse} priorities={priorities} onOpen={setSelectedStock} onBrowse={() => setView('screener')}/>}
      {view === 'watchlist' && <Watchlist universe={quotedUniverse} tickers={watchlist} priorities={priorities} sort={sort} onSort={changeSort} onOpen={setSelectedStock} onToggleSave={toggleWatchlist} onBrowse={() => setView('screener')}/>}
    </main>
  </div>
}
...
function TopBar({ view, setView }: { view: string; setView: (view: 'screener' | 'ideas' | 'watchlist') => void }) {
  ...
  return <header className="topbar"><button className="brand" onClick={() => setView('screener')}><span><TrendingUp/></span>SIGNAL</button><nav aria-label="Primary navigation"><button aria-current={view === 'screener' ? 'page' : undefined} className={view === 'screener' ? 'active' : ''} onClick={() => setView('screener')}>Screener</button><button aria-current={view === 'ideas' ? 'page' : undefined} className={view === 'ideas' ? 'active' : ''} onClick={() => setView('ideas')}>Ideas <small>NEW</small></button><button aria-current={view === 'watchlist' ? 'page' : undefined} className={view === 'watchlist' ? 'active' : ''} onClick={() => setView('watchlist')}>Watchlist</button></nav><div className="top-actions"><button aria-label="Notifications" aria-expanded={notificationsOpen} aria-controls="notification-status" onClick={() => setNotificationsOpen((open) => !open)}><Bell size={18}/></button>{notificationsOpen && <div className="notification-popover" id="notification-status" role="status"><CircleHelp size={17}/><div><strong>Notifications aren’t connected yet</strong><small>Market and watchlist alerts will appear here when enabled.</small></div></div>}<div className="avatar" aria-label="Local workspace">S</div></div></header>
}
...
function Ideas({ universe, priorities, onOpen, onBrowse }: { universe: Stock[]; priorities: MetricKey[]; onOpen: (stock: Stock) => void; onBrowse: () => void }) {
  ...
  const themes = [
    ...
    { title: 'Cash flow inflections', tag: 'Improving cash', copy: 'Accelerating free cash flow backed by positive top-line growth.', color: 'blue', picks: getRecommendations(universe.filter((s) => exceeds(s.fcfGrowth, 30) && exceeds(s.revenueGrowth, 0)), ['fcfGrowth', 'revenueGrowth']).slice(0, 3) },
  ]
  const top = getRecommendations(universe, priorities)[0]
  return <><section className="ideas-hero"><div><span className="eyebrow"><Sparkles size={14}/> Signal ideas</span><h1>A sharper place to start.</h1><p>Curated research themes built from fundamental signals—not hype.</p></div><div className="idea-feature card"><span>Highest conviction today</span><div><div className="company-logo">{top.stock.ticker[0]}</div><div><strong>{top.stock.ticker}</strong><small>{top.stock.name}</small></div><ScoreBadge score={top.score}/><button onClick={() => onOpen(top.stock)}>View thesis <ArrowRight/></button></div></div></section><section className="theme-grid">{themes.map((theme) => <article className={`theme-card card ${theme.color}`} key={theme.title}><span className="theme-tag">{theme.tag}</span><h2>{theme.title}</h2><p>{theme.copy}</p><div className="theme-picks">{theme.picks.length ? theme.picks.map((pick) => <button key={pick.stock.ticker} onClick={() => onOpen(pick.stock)}><span><i className="company-logo">{pick.stock.ticker[0]}</i><span><strong>{pick.stock.ticker}</strong><small>{pick.reason}</small></span></span><span><ScoreBadge score={pick.score}/><ArrowRight/></span></button>) : <p className="theme-empty">No companies currently qualify for this theme.</p>}</div></article>)}</section></>
}

function Watchlist({ universe, tickers, priorities, sort, onSort, onOpen, onToggleSave, onBrowse }: { universe: Stock[]; tickers: string[]; priorities: MetricKey[]; sort: SortState; onSort: (key: SortKey) => void; onOpen: (stock: Stock) => void; onToggleSave: (ticker: string) => void; onBrowse: () => void }) {
  const ranked = sortRecommendations(getRecommendations(universe.filter((stock) => tickers.includes(stock.ticker)), priorities), sort)
  return <><section className="simple-hero"><span className="eyebrow"><Bookmark size={14}/> Saved research</span><h1>Your watchlist.</h1><p>Keep the companies worth another look in one focused view.</p></section>{tickers.length === 0 ? <div className="empty card"><Bookmark size={28}/><h3>Your watchlist is empty</h3><p>Save a company from the screener to keep it here across visits.</p><button className="retry-button" onClick={onBrowse}>Browse companies</button></div> : <StockTable ranked={ranked} watchlist={tickers} sort={sort} onSort={onSort} onOpen={onOpen} onToggleSave={onToggleSave}/>}</>
}
